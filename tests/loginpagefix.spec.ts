...
test('@sanity login page title test', async ({ loginPage }) => {
...
test('@smoke @sanity user is able to login to app test', async ({ loginPage, homePage }) => {
...
for (let row of loginJSONData) {
    test(`@regression invalid login test with JSON data - ${row.username}`, async ({ loginPage }) => {
        await loginPage.doLogin(row.username, row.password);
        expect(await loginPage.isInvalidLoginErrorDisplayed()).toBeTruthy();
    });
};
